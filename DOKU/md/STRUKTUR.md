...
| ChatGPT (Web und App) | Kein Eintrag vorgesehen. ChatGPT startet keinen lokalen MCP-Server über die Standardeingabe; ein Server muss laut OpenAI „reachable through a public HTTPS endpoint or Secure MCP Tunnel“ sein. Der Tunnel braucht ein OpenAI-Plattformkonto, einen API-Schlüssel und ein Hilfsprogramm, und die Anfragen laufen über OpenAI (developers.openai.com/plugins/deploy/connect-chatgpt und developers.openai.com/api/docs/guides/secure-mcp-tunnels, abgerufen 17.09.2026). Das widerspricht dem Grundsatz „nur 127.0.0.1, kein Netz“; lokal geht es über Codex (siehe oben). | Nicht eingerichtet, nicht geprüft. |
