...
const fristEigenschaften = f => {
  ...
  return {gerechnet: termin ? null : !!(d && (r.includes(d) || (de && r.includes(de)))), belegt: /^D\d{4,}$/.test(f.quelle || '') && (termin || !!(f.ausloeser || '').trim()),
    geprueft: f.pruefstatus === 'bestätigt' && !!(f.geprueft_am || '').trim(), offene_marker: marker};
};
...
const FALL = {
  ...
  async fristen() {
    ...
    const inhalt = l.length ? `<table class="tabelle"><thead><tr><th>Datum</th><th>Frist oder Termin</th><th>Art</th><th>Prüfstatus</th><th>Grundlage</th><th></th></tr></thead><tbody>${l.map(f => { const t = tageBis(f.datum); return `<tr><td class="u-nowrap"><b>${esc(datum(f.datum))}</b><br><small>${f.pruefstatus === 'erledigt' ? 'erledigt' : t < 0 ? 'vorbei' : t === 0 ? 'heute' : 'in ' + t + ' Tagen'}</small></td><td><b>${esc(f.titel)}</b><br><small>${esc(f.ausloeser)}</small></td><td>${badge(f.art)}</td><td>${badge(f.pruefstatus, statusTon(f.pruefstatus))}<br><small>${eigenschaftenBadges(f)}</small></td><td><small>${esc(f.rechtsgrundlage)}</small>${f.quelle ? `<br><a href="${fallLink(fallId(), 'dokumente', f.quelle)}">${esc(f.quelle)}</a>` : ''}</td><td><button class="knopf klein" data-aktion="bearbeiten" data-art="frist" data-id="${esc(f.id)}">Bearbeiten</button></td></tr>`; }).join('')}</tbody></table>` : '<div class="leer"><h2>Noch keine Frist</h2><p>Jede Frist mit Auslöser, Rechtsgrundlage, Rechnung und Quelle. Der Rechner hilft bei der Rechnung.</p></div>';
