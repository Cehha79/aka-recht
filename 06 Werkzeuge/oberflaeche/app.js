...
const ZEITPUNKT = [['genau', 'genau'], ['ungefähr', 'ungefähr (ca.)'], ['zeitraum', 'Zeitraum (von bis)'], ['unbekannt', 'unbekannt (Sortierdatum)']];   // F13
// F13: Anzeige des Zeitpunkts eines Ereignisses; datum ist bei allem außer „genau“ nur das Sortierdatum
const zeitAnzeige = e => { const z = e.zeitpunkt || 'genau'; const t = e.zeitpunkt_text ? `<br><small>${esc(e.zeitpunkt_text)}</small>` : '';
  if (z === 'ungefähr') return `ca. ${esc(datum(e.datum))}${t}`;
  if (z === 'zeitraum') return `${esc(datum(e.datum))} bis ${esc(datum(e.datum_bis || e.datum))}${t}`;
  if (z === 'unbekannt') return `<span class="u-rot">unbekannt</span><br><small>einsortiert bei ${esc(datum(e.datum))}</small>${t}`;
...
const eigenschaftenBadges = f => { const e = f.eigenschaften || fristEigenschaften(f); return [
  e.gerechnet === null ? '' : badge(e.gerechnet ? 'gerechnet' : 'nicht gerechnet', e.gerechnet ? 'gruen' : 'gelb'),
  badge(e.belegt ? 'belegt' : 'nicht belegt', e.belegt ? 'gruen' : 'gelb'),
  f.pruefstatus === 'bestätigt' ? badge(e.geprueft ? 'geprüft ' + datum(f.geprueft_am) + (f.geprueft_von ? ' · ' + f.geprueft_von : '') : 'ohne Prüfdatum', e.geprueft ? 'gruen' : 'gelb') : '',
  e.ausloeser_sicher === false ? badge('Auslöser unsicher', 'rot') : '',
  e.offene_marker.length ? badge('offener Marker', 'rot') : ''].filter(Boolean).join(' '); };
...
function fristZeile(f, mitFall = true) {
  ...
  return `<div class="zeile"><div class="datumsbox ${ton}"><b>${datum(f.datum).slice(0, 5)}</b>${datum(f.datum).slice(6)}</div><div class="zeile-text"><h3>${esc(f.titel)}</h3>
    ...
    <div class="themen">${badge('Prüfstatus: ' + f.pruefstatus, statusTon(f.pruefstatus))}${(() => { const e = f.eigenschaften || fristEigenschaften(f); return (f.pruefstatus === 'bestätigt' && !e.geprueft ? ' ' + badge('ohne Prüfdatum', 'gelb') : '') + (e.offene_marker.length ? ' ' + badge('offener Marker', 'rot') : ''); })()}</div></div><div class="aktionen"><a class="knopf klein" href="${link}">Zur Akte</a></div></div>`;
}
const ZENTRALE = {
  async home() {
    ...
    const inhalt = `<div class="kacheln"><div class="kachel"><strong>${offen.length}</strong><span>offene oder ruhende Fälle</span></div><div class="kachel"><strong>${gueltig.reduce((n, c) => n + c.dokumente, 0)}</strong><span>Dokumente in den Akten</span></div>
      ...
      <div class="tafel-kopf"><h2>Deine Fälle</h2><span><button class="knopf still" data-aktion="beispiel-laden" title="Erfundener Fall zum Ausprobieren, jederzeit löschbar">Beispielfall laden</button> <a href="#seite=faelle">Alle Fälle</a></span></div><div class="raster">${offen.length ? offen.slice(0, 4).map(fallKarte).join('') : '<div class="leer"><h2>Noch kein Fall</h2><p>Lege einen Fall an, sobald du ein rechtliches Anliegen ordnen möchtest. Auch ein Strafzettel oder eine Rechnung ist ein Fall.</p><p class="u-mt14"><button class="knopf primaer" data-aktion="neuer-fall">Neuer Fall</button></p></div>'}</div>
      <div class="raster u-mt18"><section class="tafel"><h2>Nächste Fristen und Termine</h2><p class="untertitel u-mb8">Aus allen Akten. Der Prüfstatus bleibt sichtbar.</p>${fristen.length ? fristen.map(f => fristZeile(f)).join('') : '<p class="untertitel">Keine künftigen Fristen eingetragen.</p>'}</section>
      <section class="tafel"><h2>So geht es weiter</h2><p class="untertitel u-mb8">Neue Post in den Eingang legen, einem Fall zuordnen, dann in der Akte einsortieren und Fristen eintragen.</p><div class="karte-fuss"><a class="knopf" href="#seite=eingang">Posteingang</a><a class="knopf" href="#seite=quellen">Rechtsquellen</a><a class="knopf" href="#seite=bestand">Sicherung</a></div>
  ...
  async bestand() {
    ...
    const inhalt = `<section class="tafel"><div class="tafel-kopf"><h2>Bestand aller Fälle prüfen</h2>${badge('manuell')}</div><p class="untertitel u-mb10">Vergleicht jede registrierte Datei mit ihrer ersten Prüfsumme. Erkennt geänderte und fehlende Dateien. Bestätigt keine rechtliche Richtigkeit.</p><button class="knopf primaer" data-aktion="bestand-pruefen">Prüfung starten</button><div id="bestand-ergebnis"></div></section>
      <section class="tafel"><h2>Sicherung</h2><p class="untertitel">Ziel: ${esc(s.ziel || '–')}${s.ziel_hinweis ? ' (' + esc(s.ziel_hinweis) + ')' : ''}<br>Zweites Ziel: ${esc(s.zweites_ziel_eingestellt || 'keins')}${s.zweites_ziel_hinweis_cloud ? ' (' + esc(s.zweites_ziel_hinweis_cloud) + ')' : ''}<br>Die Sicherung ist eine unverschlüsselte ZIP. Lokal heißt: auf diesem Rechner. Ein Cloud-Ordner wird vom Betriebssystem hochgeladen. Was deine KI liest, verarbeitet ihr Anbieter. Ziele in den Einstellungen.</p>
      ${s.vorhanden ? `<p class="pfad">${esc(s.pfad)}</p><p class="untertitel">Erstellt ${esc(datum(s.zeit))} · ${s.dateien} Dateien · ${groesse(s.groesse)} · ${badge(s.unveraendert ? 'seit Prüfung unverändert' : 'seit Prüfung verändert', s.unveraendert ? 'gruen' : 'rot')}${s.zweites_ziel ? `<br>Kopie: ${esc(s.zweites_ziel)} ${badge(s.zweites_ziel_unveraendert ? 'unverändert' : 'verändert oder fehlt', s.zweites_ziel_unveraendert ? 'gruen' : 'rot')}` : ''}${s.zweites_ziel_hinweis ? `<br>${esc(s.zweites_ziel_hinweis)}` : ''}</p>` : '<div class="hinweis gelb">Noch keine Sicherung mit dem neuen Dienst dokumentiert.</div>'}
      <div class="karte-fuss"><button class="knopf primaer" data-aktion="sicherung">Geprüfte Sicherung erstellen</button>${s.vorhanden ? '<button class="knopf" data-aktion="sicherung-probe">Wiederherstellung prüfen</button>' : ''}</div><p class="untertitel">Die ZIP wird vollständig gelesen und mit dem Arbeitsstand verglichen. Eine Kopie geht an das zweite Ziel, wenn es erreichbar ist. Frühere Sicherungen bleiben erhalten. „Wiederherstellung prüfen“ entpackt die letzte Sicherung in einen Zwischenordner, prüft Akten und Prüfsummen und räumt ihn wieder ab; die Mappe bleibt unberührt.</p><div id="probe-ergebnis"></div></section>`;
  ...
  async einstellungen() {
    ...
    const inhalt = `<form id="einstellungen" class="tafel"><h2>Sicherung</h2>
      ...
      <h2>Absender für Entwürfe</h2>
      <p class="untertitel">Wird beim Anlegen eines Entwurfs aus einer Vorlage in „Von:“ und die Unterschrift gesetzt. Hat ein Fall einen Beteiligten mit Rolle „Ich“ und Anschrift, gilt dort dieser. Bleibt in zentrale.json auf diesem Rechner.</p>
      <div class="feld-reihe">${feld('ab_name', 'Name', (e.einstellungen.absender || {}).name || '')}${feld('ab_strasse', 'Straße und Hausnummer', (e.einstellungen.absender || {}).strasse || '')}</div>
      <div class="feld-reihe drei">${feld('ab_plz_ort', 'PLZ und Ort', (e.einstellungen.absender || {}).plz_ort || '')}${feld('ab_telefon', 'Telefon', (e.einstellungen.absender || {}).telefon || '')}${feld('ab_email', 'E-Mail', (e.einstellungen.absender || {}).email || '')}</div>
      <h2>Fristen</h2>
  ...
  async anleitung() {
    const inhalt = `<article class="tafel anleitung"><h2>Was AKA Recht ist</h2><p>Ein lokaler Ordner mit einer Oberfläche im Browser. Jede Rechtssache ist ein Fall mit fester Kennung (R-0001, R-0002 …), egal ob Arbeitsrecht, Bußgeld, Miete, Vertrag oder Behörde. Alles bleibt auf diesem Rechner.</p>
      ...
      <h2>KI anbinden (MCP)</h2><p>MCP ist die Schnittstelle, über die eine KI die Werkzeuge der Mappe aufruft. Der Server ist die Datei „06 Werkzeuge/dienst/mcp_server.py“; er braucht nur Python 3.</p><ul><li><b>Claude Code:</b> Sitzung im Projektordner starten. Die Datei „.mcp.json“ liegt bei; beim ersten Start fragt Claude Code, ob es den Server aus dem Projekt laden darf. Prüfen mit „/mcp“. Skills wie „/fristencheck R-0001“ liegen unter „.claude/skills/“.</li><li><b>Codex:</b> einmalig im Terminal „codex mcp add aka-recht -- python3 "voller Pfad zu mcp_server.py"“ eingeben (unter Windows „python“ statt „python3“), dann Codex im Projektordner starten und mit „/mcp“ prüfen. Die beigelegte „.codex/config.toml“ lädt Codex nur, wenn genau dieser Projektordner in „~/.codex/config.toml“ als vertraut eingetragen ist; dann ist „codex mcp add“ nicht nötig. Skills liegen unter „.agents/skills/“ und werden mit „$fristencheck“ aufgerufen.</li><li><b>Claude Desktop:</b> Menü Claude, Einstellungen, Entwickler, „Konfiguration bearbeiten“. In der Datei claude_desktop_config.json unter „mcpServers“ einen Eintrag „aka-recht“ mit command „python3“ (unter Windows „python“) und args [voller Pfad zu mcp_server.py] eintragen, Pfade absolut. Claude Desktop danach ganz beenden und neu starten.</li><li><b>Ohne MCP:</b> Jede KI, die Befehle ausführen darf, nutzt „python3 06 Werkzeuge/dienst/cli.py liste“.</li></ul>
...
// ---------------------------------------------------------------- Seiten der Fallakte
function dokZeile(d) {
  return `<div class="dok-zeile ${S.auswahl === d.id ? 'gewaehlt' : ''}" data-dok="${esc(d.id)}" tabindex="0" role="button"><div class="dok-zelle"><div class="dateisymbol ${esc(d.typ)}">${esc(d.typ || '?')}</div><div class="u-minw0"><div class="dok-titel">${esc(d.titel)}</div><div class="dok-unter">${esc(d.id)}${d.anlage ? ' · ' + esc(d.anlage) : ''} · ${esc(d.pfad)}${d.fehlt ? ' · <b class="u-rot">Datei fehlt</b>' : ''}</div>${d.themen.length ? `<div class="themen">${d.themen.map(t => `<span class="thema">${esc(t)}</span>`).join('')}</div>` : ''}</div></div>
...
const FALL = {
  async uebersicht() {
    ...
    const inhalt = `<section class="tafel"><div class="tafel-kopf"><div><span class="kennung">${esc(f.id)}</span> ${badge(f.bereich)} ${badge(f.status, f.status === 'offen' ? 'gruen' : '')}</div><button class="knopf klein" data-aktion="fall-bearbeiten">Fall bearbeiten</button></div>
      <div class="raster drei u-mt8"><div class="feld-anzeige"><b>Rolle</b><span>${esc(f.rolle || 'noch offen')}</span></div><div class="feld-anzeige"><b>Ziel</b><span>${esc(f.ziel || 'noch offen')}</span></div><div class="feld-anzeige"><b>Verfahren</b><span>${a.verfahren.map(v => `${esc(v.art)}${v.aktenzeichen ? ' · ' + esc(v.aktenzeichen) : ''}`).join('<br>') || 'noch keins'}</span></div></div>
      ${f.untertitel ? `<p class="untertitel u-m0">${esc(f.untertitel)}</p>` : ''}</section>
      <div class="kacheln u-mt18"><div class="kachel"><strong>${dokListe().length}</strong><span>Dokumente</span></div><div class="kachel"><strong>${a.aufgaben.filter(x => !x.erledigt).length}</strong><span>offene Aufgaben</span></div><div class="kachel"><strong>${a.fristen.filter(x => x.pruefstatus !== 'erledigt').length}</strong><span>Fristen und Termine</span></div><div class="kachel"><strong>${a.ereignisse.length}</strong><span>Ereignisse</span></div></div>
      <div class="raster"><section class="tafel"><div class="tafel-kopf"><h2>Nächste Fristen</h2><a href="${fallLink(f.id, 'fristen')}">Alle</a></div>${fristen.length ? fristen.map(x => fristZeile(x, false)).join('') : '<p class="untertitel u-m0">Keine offenen Fristen.</p>'}</section>
      <section class="tafel"><div class="tafel-kopf"><h2>Offene Aufgaben</h2><a href="${fallLink(f.id, 'aufgaben')}">Alle</a></div>${aufgaben.length ? aufgaben.map(x => `<div class="zeile"><div class="zeile-text"><h3>${esc(x.titel)}</h3><p>${x.faellig ? 'fällig ' + esc(datum(x.faellig)) + ' · ' : ''}${esc(x.detail)}</p></div></div>`).join('') : '<p class="untertitel u-m0">Keine offenen Aufgaben.</p>'}</section></div>
      <div class="raster u-mt18"><section class="tafel"><h2>Aktenbereiche</h2><div class="bereiche">${gruppen}</div></section>
  ...
  async fristen() {
    ...
    const inhalt = l.length ? `<table class="tabelle"><thead><tr><th>Datum</th><th>Frist oder Termin</th><th>Art</th><th>Prüfstatus</th><th>Grundlage</th><th></th></tr></thead><tbody>${l.map(f => { const t = tageBis(f.datum); return `<tr><td class="u-nowrap"><b>${esc(datum(f.datum))}</b><br><small>${f.pruefstatus === 'erledigt' ? 'erledigt' : t < 0 ? 'vorbei' : t === 0 ? 'heute' : 'in ' + t + ' Tagen'}</small></td><td><b>${esc(f.titel)}</b><br><small>${esc(f.ausloeser)}${f.verfahren ? ' · ' + esc(f.verfahren) : ''}${f.ausloeser_ereignis ? ' · Auslöser ' + esc(f.ausloeser_ereignis) : ''}</small></td><td>${badge(f.art)}</td><td>${badge(f.pruefstatus, statusTon(f.pruefstatus))}<br><small>${eigenschaftenBadges(f)}</small></td><td><small>${esc(f.rechtsgrundlage)}</small>${f.quelle ? `<br><a href="${fallLink(fallId(), 'dokumente', f.quelle)}">${esc(f.quelle)}</a>` : ''}</td><td><button class="knopf klein" data-aktion="bearbeiten" data-art="frist" data-id="${esc(f.id)}">Bearbeiten</button></td></tr>`; }).join('')}</tbody></table>` : '<div class="leer"><h2>Noch keine Frist</h2><p>Jede Frist mit Auslöser, Rechtsgrundlage, Rechnung und Quelle. Der Rechner hilft bei der Rechnung.</p></div>';
  ...
  async entwuerfe() {
    ...
    const eingefroren = w => (w.fassungen || []).filter(x => x.kopie_dokument).map(x => `Fassung ${x.fassung} ${esc(x.status)} ${esc(datum(x.zeit))} <a href="${fallLink(fallId(), 'dokumente', x.kopie_dokument)}">${esc(x.kopie_dokument)}</a>`).join(' · ');
    const inhalt = `<section class="tafel"><h2>Entwürfe mit Fassung</h2>${l.length ? l.map(w => `<div class="zeile"><div class="zeile-text"><h3>${esc(w.titel)}</h3><p>Fassung ${w.fassung} · ${esc(w.datei)}${w.versandt_als ? ` · versandt als <a href="${fallLink(fallId(), 'dokumente', w.versandt_als)}">${esc(w.versandt_als)}</a>` : ''}</p>${eingefroren(w) ? `<p>Eingefroren: ${eingefroren(w)}</p>` : ''}</div>${badge(w.status, w.status === 'versandt' ? 'gruen' : w.status === 'geprüft' ? 'blau' : 'gelb')}<div class="aktionen"><button class="knopf klein" data-aktion="bearbeiten" data-art="entwurf" data-id="${esc(w.id)}">Bearbeiten</button></div></div>`).join('') : '<p class="untertitel u-m0">Noch kein Entwurf erfasst. Ein Entwurf bleibt Entwurf, bis der Versand belegt ist.</p>'}</section>
      <section class="tafel"><h2>Dateien in 06 Entwürfe</h2>${dateien.length ? dateien.map(d => `<div class="zeile"><div class="zeile-text"><h3><a href="${fallLink(fallId(), 'dokumente', d.id)}">${esc(d.titel)}</a></h3><p>${esc(d.id)} · ${esc(d.pfad)}</p></div>${badge(d.stand, statusTon(d.stand))}</div>`).join('') : '<p class="untertitel u-m0">Keine Dateien im Ordner 06 Entwürfe.</p>'}</section>`;
    return ['Entwürfe', 'Noch nicht versandte Texte. Dateiname endet auf _ENTWURF. Versand nur nach Freigabe, dann Beleg als Dokument.', inhalt, '<button class="knopf" data-aktion="vorlage">Entwurf aus Vorlage</button><button class="knopf primaer" data-aktion="neu" data-art="entwurf">Entwurf erfassen</button>'];
  },
  async anlagen() {
    ...
    const inhalt = `<section class="tafel"><h2>Anlagenverzeichnis</h2><p class="untertitel u-mb10">Zuordnung vorhandener Dateien zu Anlagenkennungen. Kein Nachweis, welcher Satz tatsächlich eingereicht wurde.</p>${anl.length ? `<table class="tabelle"><thead><tr><th>Anlage</th><th>Dokument</th><th>Datum</th><th>Stand</th></tr></thead><tbody>${anl.map(d => `<tr class="klick" data-dok-link="${esc(d.id)}"><td><b>${esc(d.anlage)}</b></td><td>${esc(d.titel)}<br><small>${esc(d.id)} · ${esc(d.pfad)}</small></td><td>${esc(datum(d.datum))}</td><td>${badge(d.stand, statusTon(d.stand))}</td></tr>`).join('')}</tbody></table>` : '<p class="untertitel u-m0">Noch keine Anlagenkennungen vergeben. Über „Ordnen“ am Dokument.</p>'}</section>
      <section class="tafel"><h2>Beweise (05 Beweise)</h2>${bew.length ? `<div class="raster drei">${bew.map(d => `<article class="karte"><span class="kennung">${esc(d.id)}</span><h2 class="u-fs95"><a href="${fallLink(fallId(), 'dokumente', d.id)}">${esc(d.titel)}</a></h2><p>${esc(d.pfad)}</p></article>`).join('')}</div>` : '<p class="untertitel u-m0">Keine Dateien in 05 Beweise.</p>'}</section>`;
...
async function vorschau(zeigen) {
  ...
  if (d.fehlt) inhalt = '<div class="hinweis rot">Die Datei fehlt am registrierten Ort. Bestand neu einlesen oder im ' + DATEIMANAGER + ' nachsehen.</div>';
    ...
    try { const t = await api.get(`/api/fall/${id}/text/${d.id}`); inhalt = `<div class="hinweis u-m12-16"><b>Textquelle:</b> ${esc(t.textquelle_text || '')}${t.seiten ? ` · ${t.seiten} Seite(n)` : ''}${t.textstand ? ` · <b>Textstand:</b> ${esc(t.textstand)}` : ''}${t.hinweis ? `<br>${esc(t.hinweis)}` : ''}</div>` + (['bild', 'kein-text'].includes(t.textquelle) ? `<div class="hinweis u-m12-16"><button class="knopf klein primaer" data-aktion="texterkennung">Texterkennung starten</button> Liest die Schrift mit dem Programm tesseract auf diesem Rechner und legt das Ergebnis als eigene Textdatei unter 07 Recherche an. Das Original bleibt unverändert; erkannter Text ist eine Ableitung.</div>` : t.texterkennung ? `<div class="hinweis u-m12-16">Der Text stammt aus der Texterkennung <a href="${fallLink(id, 'dokumente', t.texterkennung)}">${esc(t.texterkennung)}</a>. Zahlen, Daten und Namen am Original prüfen.</div>` : '') + `<pre>${esc(t.text || '(kein Text ausgelesen)')}</pre>`; } catch (e) { inhalt = `<div class="hinweis rot">${esc(e.message)}</div>`; }
  } else {
...
const FORMULARE = {
  ...
  ereignis: {liste: 'ereignisse', kennung: 'E', titel: 'Ereignis', felder: e => `<div class="feld-reihe">${feld('datum', 'Datum', e.datum, 'date')}${feld('art', 'Art', e.art, 'select', {optionen: opt(EREIGNIS_ART, e.art || 'Vermerk')})}</div>${feld('titel', 'Kurztitel', e.titel)}<div class="feld-reihe drei">${feld('zeitpunkt', 'Zeitpunkt', e.zeitpunkt || 'genau', 'select', {optionen: opt(ZEITPUNKT, e.zeitpunkt || 'genau'), hinweis: 'Bei allem außer „genau“ ist das Datum nur das Sortierdatum.'})}${feld('datum_bis', 'Bis (Zeitraum)', e.datum_bis, 'date')}${feld('zeitpunkt_text', 'Was ist bekannt', e.zeitpunkt_text, 'text', {hinweis: 'z. B. „Anfang September laut Kollegin“'})}</div>${feld('quelle', 'Quelle', e.quelle, 'select', {optionen: dokOptionen(e.quelle)})}${feld('detail', 'Einzelheiten', e.detail, 'textarea', {hinweis: 'Was steht im Dokument, was ist eigene Angabe, was ist Behauptung der Gegenseite?'})}`,
  ...
  frist: {liste: 'fristen', kennung: 'F', titel: 'Frist oder Termin', felder: x => rechnerHtml(x, true) + `
    <div class="feld-reihe">${feld('datum', 'Fristende oder Termin', x.datum, 'date')}${feld('art', 'Art', x.art, 'select', {optionen: opt(FRIST_ART, x.art || 'gesetzlich')})}</div>${feld('titel', 'Kurztitel', x.titel)}<div class="feld-reihe">${feld('verfahren', 'Verfahren', x.verfahren, 'select', {optionen: opt(akte().verfahren.map(v => [v.id, v.id + ' · ' + v.art]), x.verfahren, 'Keine Angabe')})}${feld('ausloeser_ereignis', 'Auslöser-Ereignis', x.ausloeser_ereignis, 'select', {optionen: opt([...akte().ereignisse].sort((a, b) => a.datum.localeCompare(b.datum)).map(e => [e.id, e.id + ' · ' + datum(e.datum) + ((e.zeitpunkt || 'genau') !== 'genau' ? ' (' + e.zeitpunkt + ')' : '') + ' · ' + e.titel]), x.ausloeser_ereignis, 'Keine Angabe'), hinweis: '„bestätigt“ nur mit genau datiertem Ereignis.'})}</div>${feld('ausloeser', 'Auslöser und Zugang', x.ausloeser, 'text', {hinweis: 'z. B. Zustellung am 05.09.2026, Umschlag D0004'})}${feld('rechtsgrundlage', 'Rechtsgrundlage', x.rechtsgrundlage, 'text', {hinweis: 'Norm mit Absatz und Gesetz, z. B. § 67 Abs. 1 OWiG'})}${feld('berechnung', 'Rechnung', x.berechnung, 'textarea')}<div class="feld-reihe">${feld('pruefstatus', 'Prüfstatus', x.pruefstatus, 'select', {optionen: opt(FRIST_STATUS, x.pruefstatus || 'offen'), hinweis: '„bestätigt“ nur mit Auslöser, Grundlage, Rechnung und Quelle.'})}${feld('quelle', 'Quelle', x.quelle, 'select', {optionen: dokOptionen(x.quelle)})}</div>${feld('geprueft_von', 'Geprüft von', x.geprueft_von, 'text', {hinweis: 'Nur bei „bestätigt“: wer Rechnung, Beleg und Grundlage geprüft hat. Das Prüfdatum setzt die Mappe beim Speichern; die Rechnung muss das Fristende nennen, offene Marker sperren die Bestätigung.'})}<input type="hidden" name="geprueft_am" value="${esc(x.geprueft_am || '')}">`,
...
function eintragDialog(art, id) {
  ...
  dialog((id ? 'Bearbeiten: ' : 'Neu: ') + F.titel + (id ? ' ' + id : ''), F.felder(werte) + (id ? `<p><button type="button" class="knopf klein" data-aktion="eintrag-entfernen" data-art="${art}" data-id="${esc(id)}">Eintrag entfernen</button> <small class="u-muted">Nur möglich, wenn nichts darauf verweist.</small></p>` : ''),
...
async function vorlageDialog() {
  ...
  dialog('Entwurf aus Vorlage', `${feld('vorlage', 'Vorlage', '', 'select', {optionen: opt(vorlagen.map(v => v.name), vorlagen[0] ? vorlagen[0].name : '')})}${feld('ziel', 'Dateiname', '', 'text', {hinweis: 'optional, unter 06 Entwürfe; Standard JJJJ-MM-TT_<Vorlage>_ENTWURF.md'})}<div class="hinweis">Absender, Unterschrift und Datum kommen aus den Einstellungen oder vom Beteiligten mit Rolle „Ich“. Alle anderen Platzhalter 【…】 bleiben zum Ausfüllen. Nichts wird überschrieben.</div>`,
    async f => { const r = await api.werkzeug('vorlage_fuellen', {fall: fallId(), vorlage: f.get('vorlage'), ziel: f.get('ziel').trim()}); await ladeFall(fallId()); toast(`Entwurf angelegt: ${r.datei}` + (r.absender ? ` · Absender: ${r.absender_quelle}` : ' · kein Absender hinterlegt') + ` · ${r.offene_platzhalter} Platzhalter offen.`); await render(); }, 'Anlegen');
}
...
async function sicherungProbe(b) {
  const ziel = $('#probe-ergebnis'); b.disabled = true; ziel.innerHTML = '<div class="hinweis">Probe läuft: Archiv wird entpackt und geprüft …</div>';
  try {
    const r = await api.post('/api/sicherung/probe', {});
    const faelle = r.faelle.map(c => `${esc(c.fall)}: ${c.geprueft} Dateien stimmen, ${c.veraendert.length} verändert, ${c.fehlend.length} fehlend, ${c.schema_fehler.length} Schemafehler`).join('<br>');
    ziel.innerHTML = `<div class="ergebnisblock ${r.bestanden ? 'gut' : 'schlecht'}">${r.bestanden ? 'Wiederherstellung geprüft: bestanden.' : 'Wiederherstellung geprüft: nicht bestanden.'}<br>${r.dateien} Dateien entpackt${r.pruefsummendatei === false ? ', Prüfsummendatei passt nicht' : ''}.<br>${faelle}${r.fehler.length ? '<br>' + r.fehler.map(esc).join('<br>') : ''}${(r.hinweise || []).length ? '<br>' + r.hinweise.map(esc).join('<br>') : ''}</div><p class="untertitel">${esc(r.hinweis)}</p>`;
  } catch (e) { ziel.innerHTML = `<div class="hinweis rot">${esc(e.message)}</div>`; }
  finally { b.disabled = false; }
}
...
document.addEventListener('click', async e => {
  ...
  try {
    ...
    else if (a === 'texterkennung') { b.disabled = true; toast('Texterkennung läuft, das kann je Seite einige Sekunden dauern …'); const r = await api.werkzeug('texterkennung', {fall: fallId(), dokument: S.auswahl}); await ladeFall(fallId()); toast(`Texterkennung angelegt: ${r.texterkennung}, ${r.seiten} Seite(n), ${r.zeichen} Zeichen. Zahlen, Daten und Namen am Original prüfen.`); await render(); }
