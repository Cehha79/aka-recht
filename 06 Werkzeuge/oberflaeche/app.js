...
const ZENTRALE = {
  async home() {
    ...
    // N04: Ein abgelaufenes Datum darf einen offenen Vorgang nicht aus dem Blick nehmen — überfällige zuerst
    const offeneFristen = fristenAllerFaelle().filter(f => f.pruefstatus !== 'erledigt');
    const ueberfaellig = offeneFristen.filter(f => f.datum < heute());
    const fristen = offeneFristen.filter(f => f.datum >= heute()).slice(0, 6);
    const inhalt = `<div class="kacheln"><div class="kachel"><strong>${offen.length}</strong><span>${esc(t('home.offene_faelle'))}</span></div><div class="kachel"><strong>${gueltig.reduce((n, c) => n + c.dokumente, 0)}</strong><span>${esc(t('home.dokumente'))}</span></div>
      ...
      <div class="raster u-mt18"><section class="tafel">${ueberfaellig.length ? `<h2>${esc(t('home.ueberfaellig', {n: ueberfaellig.length}))}</h2><p class="untertitel u-mb8">${esc(t('home.ueberfaellig_text'))}</p>${ueberfaellig.slice(0, 6).map(f => fristZeile(f)).join('')}${ueberfaellig.length > 6 ? `<p class="untertitel">${esc(t('home.ueberfaellig_mehr', {n: ueberfaellig.length - 6}))}</p>` : ''}<h2 class="u-mt18">${esc(t('home.naechste_fristen'))}</h2>` : `<h2>${esc(t('home.naechste_fristen'))}</h2>`}<p class="untertitel u-mb8">${esc(t('home.naechste_fristen_text'))}</p>${fristen.length ? fristen.map(f => fristZeile(f)).join('') : `<p class="untertitel">${esc(t('home.keine_fristen'))}</p>`}</section>
